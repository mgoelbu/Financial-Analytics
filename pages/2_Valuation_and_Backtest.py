...
with tab1:
    ...
    if ticker_input in ticker_data.values:
        ...
        col1.metric("EPS (2024)", f"{eps_2024:.2f}" if eps_valid else "N/A")
...
with tab2:
    ...
    if ticker_input in ticker_data.values:
        # Everything from your Backtest code, unmodified, 
        # except remove the ticker_input selectbox and page_config.
        idx      = ticker_data[ticker_data == ticker_input].index[0]
        ...
        for year in range(2010, 2024):
            ...
            if (year+1 in actual_price.index and pd.notna(actual_price.get(year+1))):
                m = "Up" if actual_price[year+1] > actual_price[year] else "Down"
                if model_pred == m: correct_predictions += 1
                total_predictions += 1
        ...
        # Final Prediction, Gsubind & Global Accuracy...
        # (Insert your remaining Backtest code blocks here unmodified)
