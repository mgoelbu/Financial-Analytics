...
with tab3:
    ...
    if ticker_input in ticker_data.values:
        ...
        try:
            ...
            fig_snap = go.Figure()
            fig_snap.add_trace(go.Scatter(x=hist.index, y=hist["Close"], mode='lines', name='Close Price'))
            ...
            st.markdown("""
            <style>
                .metric-table {display: grid; grid-template-columns: repeat(4, 1fr); gap: 10px;
                                border: 1px solid #eee; border-radius: 8px;}
                .metric-item {border: 1px solid #ddd; border-radius: 6px; padding: 10px;
                              text-align: center; font-size: 14px; background: #fafafa;}
                .metric-item b {font-size: 16px;}
            </style>""", unsafe_allow_html=True)

            html = "<div class='metric-table'>" + "".join([
